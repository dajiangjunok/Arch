...
普通码与优惠码绑定同一个分销商。只有 Single Week Access 项目（含一周、两周、三周套餐）的有效付费推荐计入阶梯，按分销商合并计算，每份推荐只计一次。例如一笔 $9,799 普通订单加一笔 $8,500 优惠订单，在默认 9% 档位下累计佣金为 $1,646.91；再增加一笔 $9,799 普通订单后达到 3 笔，按默认 15% 重算累计佣金为 $4,214.70，并补记差额。

所有 Fellowship 套餐通过普通邀请码归属分销商，按实付金额减去退款后的净额固定返佣 10%。Fellowship 的人数和金额均不参与阶梯统计，修改阶梯配置也不会改变其 10% 比例。例如两笔 $8,500 Single Week Access 订单加一笔 $1,500 Fellowship 订单，阶梯人数仍为 2，累计佣金为 $1,530 + $150 = $1,680。

实付 $8,500 时，各档对应 $765 / $1,275 / $1,700 / $2,550。计算基数继续使用实付金额减去已退款金额，不扣 Stripe 手续费。全额退款会移除相应有效付费推荐，并按现有规则重算档位及佣金。

部署本次佣金变更前，按顺序执行至 `supabase/migrations/019_fellowship_fixed_commissions.sql`，再部署应用。该迁移保留历史佣金及已结算状态，以新增差额记录校准两类佣金，负差额自动批准并计入待结算余额。手动撤销的金额仍按原有规则在同一分销商、同一币种下永久扣除（优先抵扣原佣金类别，超出部分抵扣另一类别）；停用分销商仍会将佣金权益归零。
